...
> **MJ Dev Manager subsystem (the `packages/orchestrator` engine, `mjdev` CLI, instance/open-app dev-linking):** before working on any of it, read [`docs/mj-dev-manager.md`](docs/mj-dev-manager.md) (and [`docs/mj-dev-manager-maintenance.md`](docs/mj-dev-manager-maintenance.md) for the parity-compliance watch-list when MJ/apps update). It holds the load-bearing conventions (single-copy invariant, "reproduce the shell / delegate the steps", Option Y worktrees, one-façade-three-surfaces), the slice status, and the **validation strategy**. Key practice: live (runtime) validation is wall-clock + context heavy, so reuse a long-lived **golden instance** (recipe in [`fixtures/golden-instance.yaml`](fixtures/golden-instance.yaml), pinned to a stable MJ release) rather than re-paying `create + setup all` each run — and never commit the instance itself, only the recipe. Validate live only when a slice introduces new runtime semantics; batch plumbing slices behind one boot; run the heavy validate-as-install tier only before a PR. **Open work / what's next lives in [`docs/action-items/mj-dev-manager-backlog.md`](docs/action-items/mj-dev-manager-backlog.md) — read it before starting, and keep it current as you finish or discover items.**
>
> **Standing rule for all editors of this subsystem (always, no exceptions):**
>
...
> 2. **Record architectural/structural decisions in [`docs/mj-dev-manager-decisions.md`](docs/mj-dev-manager-decisions.md).**
...
>    in the repo, not just in chat. A reviewer should never have to reverse-engineer intent.
