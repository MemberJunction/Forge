...
function managedAgentsBlock(): string {
  return [
    ...
    '',
    '**Open your first response with the greeting in** @.mjdev-docs/WELCOME.md — it introduces the',
    'harness (CLI **and** GUI), lists what you can do, and asks the one startup question: **how many',
    '**heavy slots** the machine can run** (a heavy slot = one long-running, high-compute task at a',
    'time — a full MJ `turbo build`, an instance `create` + `setup all`, an `app build-all`, or a',
    'Playwright/e2e run). Keep concurrent heavy work within that budget; run it in the **background**',
    'and parallelize up to it (see ORCHESTRATION.md "Heavy slots").',
    '',
