...
8. **Pure topology per instance — NEVER mix install + dev-link (decided 2026-06-21).** An instance is either all-dev-link (an app **and its open-app dependency closure**) or all-install. A dev-linked workspace member depending on an _installed_ (registry) app crashes npm's arborist (registry `peerDependencies` resolving onto workspace `@memberjunction/*` symlinks — a known npm bug). Both pure topologies are bug-free; only the mix breaks. **Dev-link is primary** (identical runtime parity to install — registration/schema/migrations/config/record; the only delta, published-artifact resolution, is covered by the `validate-as-install` tier). A dev-linked app's deps **default to dev-link** (same-mode closure). The **install path is retained** for the `validate-as-install` tier (+ future "management" instances), **not** for dependencies of live dev-linked apps. Guard against the mix; don't paper over it with `--legacy-peer-deps`. MJ-version compatibility is gated by the manifest `mjVersionRange` (the authoritative alert), not npm peers. **As MJ/apps update, the reproduced-parity surface must be re-verified — see [`mj-dev-manager-maintenance.md`](mj-dev-manager-maintenance.md).**
...
- **2026-06-22** — **CAPSTONE: runtime registration PROVEN LIVE.** Built + booted the pure dev-link closure (openapp-dev): MJAPI clean (`Server ready :4020`), and `mj codegen manifest` (MJAPI prestart) scanned the dev-linked local source → emitted bizapps `@RegisterClass` classes into the manifest MJAPI imports (`index.ts:17`) → registered in the factory from local source. Phase B thesis proven with a real first-party multi-package app. F-boot answered for 5.40.2: registration via manifest works; `dynamicPackages.server` StartupExport is a no-op (no consumer). Follow-ups: `buildApp` needs dep-order/retry; dev-link should run `mj codegen manifest` to pre-converge MJAPI deps. Remaining: edit-see-live + GUI e2e + validate tiers.
- **2026-06-21** — **Pure-topology decision VALIDATED LIVE.** Dev-linked `bizapps-common` + `bizapps-accounting` (multi-package closure, accounting→common) into `openapp-dev` with **zero npm-arborist crashes**, single-copy held across MJAPI + both members, topo-ordered client bootstrap. Confirms convention #8: pure dev-link closure works where the mixed topology crashed. Remaining: build + boot + prove `@RegisterClass` + edit-see-live (the see-it-live capstone).
- **2026-06-21** — Install path I1–I3 (plain-install via real `InstallApp`, dependency popup, recents) + reserved-`__`-schema support built & committed. Capstone: `bizapps-common` installed live; dev-link of multi-package `bizapps-accounting` surfaced the npm-arborist mixed-topology crash. **Decision: abandon mixed topology → pure dev-link (primary) / pure install (validate tier), never mix** (convention #8). Pending: flip the dependency-popup default to same-mode (dev-link deps of a dev-linked app) + add a mix-guard; then complete the capstone via a pure dev-link closure. Added [`mj-dev-manager-maintenance.md`](mj-dev-manager-maintenance.md).
